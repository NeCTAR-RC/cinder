...
class API(base.Base):
    ...
    def list_availability_zones(self, context, enable_cache=False,
        ...
        quota_values = QUOTAS.get_project_quotas(context, context.project_id)
        no_quota_zones = [q.split('_')[-1] for q, v in quota_values.items()
                          if q.startswith('volumes_') and v['limit'] == -1]
        for az in azs:
            az_name = az['name'].split('-')[0]
            if az_name in no_quota_zones:
                az['available'] = False
